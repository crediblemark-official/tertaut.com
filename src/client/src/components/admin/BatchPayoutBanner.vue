...
<template>
  <div class="space-y-3">
    ...
    >
      <div class="flex items-center gap-2 text-xs">
        ...
        <span class="text-jetblack/80">
          <strong>{{ stats?.pendingDisbursementsCount }}</strong> transaksi (total
          <strong>Rp {{ (stats?.pendingDisbursementsAmount || 0).toLocaleString("id-ID") }}</strong
          >) siap dicairkan ke builder.
        </span>
      </div>
    ...
    <section
      v-if="payoutResult"
      class="p-3 rounded-lg bg-forest/10 border border-forest/25 text-forest flex items-center justify-between text-xs font-bold"
    >
      <div class="flex items-center gap-2">
        <CheckCircle2 class="w-4 h-4 shrink-0" />
        <span
          >Berhasil memproses {{ payoutResult.processedCount }} transaksi (total Rp
          {{ payoutResult.totalDisbursed.toLocaleString("id-ID") }}).</span
        >
      </div>
      <span class="text-[10px] text-forest/70 font-mono">{{
