...
<template>
  <div class="grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-jetblack/10 border-b border-jetblack/10 pb-6 pt-2">
    <!-- Left Column: Builder Highlights -->
    <div class="lg:pr-6 space-y-3 pb-6 lg:pb-0">
      <div class="flex items-center justify-between">
        <h2 class="text-xs font-bold text-jetblack uppercase tracking-wide">Builder Terdaftar Terbaru</h2>
        <button @click="emit('selectTab', 'builders')" class="text-[11px] font-bold text-gold hover:underline cursor-pointer">
      ...
      <div class="space-y-2 divide-y divide-jetblack/10">
        <div v-for="b in builders.slice(0, 5)" :key="b.id" class="flex items-center justify-between pt-2 text-xs">
          ...
          <div class="text-right">
            <div class="font-mono font-bold text-forest">Rp {{ (b.totalGMV || 0).toLocaleString('id-ID') }}</div>
    ...
    <div class="lg:pl-6 space-y-3 pt-6 lg:pt-0">
      <div class="flex items-center justify-between">
        <h2 class="text-xs font-bold text-jetblack uppercase tracking-wide">Transaksi Global Terkini</h2>
        <button @click="emit('selectTab', 'ledger')" class="text-[11px] font-bold text-gold hover:underline cursor-pointer">
      ...
      <div class="space-y-2 divide-y divide-jetblack/10">
        <div v-for="t in transactions.slice(0, 5)" :key="t.id" class="flex items-center justify-between pt-2 text-xs">
          <div>
            <div class="font-mono font-bold text-jetblack truncate max-w-[200px]">{{ t.customerEmail }}</div>
            <div class="text-[10px] text-jetblack/50 font-mono">{{ t.id }}</div>
          </div>
          <div class="text-right">
            <div class="font-mono font-bold text-jetblack">Rp {{ t.grossAmount.toLocaleString('id-ID') }}</div>
