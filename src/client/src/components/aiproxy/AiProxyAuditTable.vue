...
<template>
  <div class="space-y-4">
    ...
    <div class="hidden sm:block overflow-x-auto w-full top-scrollbar">
      <table class="w-full text-left text-xs whitespace-nowrap">
        <thead class="border-b border-[#111111]/10 text-[#111111]/60 font-bold uppercase text-[10px]">
          <tr>
            <th class="py-2.5 pr-3 pl-0">Waktu</th>
            ...
            <th class="py-2.5 pl-3 pr-0 text-right">Status Shield</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-[#111111]/5">
          ...
          <tr v-for="log in proxyLogs" :key="log.id" class="hover:bg-[#111111]/[0.02]">
            <td class="py-3 pr-3 pl-0 font-mono text-[11px] text-[#111111]/60">
              {{ new Date(log.createdAt).toLocaleTimeString('id-ID') }}
            </td>
            <td class="py-3 px-3 font-mono font-bold text-[#111111]">
              {{ log.licenseKey || 'Direct' }}
            </td>
            <td class="py-3 px-3 text-[#111111]">
            ...
            <td class="py-3 px-3 font-mono text-[11px] text-[#0F4C3A] font-bold">
              {{ log.latencyMs }} ms
            </td>
    ...
    <div class="sm:hidden divide-y divide-[#111111]/10">
      <div v-for="log in proxyLogs" :key="log.id" class="py-3 space-y-1.5">
