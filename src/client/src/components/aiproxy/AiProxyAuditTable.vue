...
<template>
  <div class="space-y-4">
    <!-- Header -->
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2 border-b border-jetblack/10">
      <h2 class="text-sm font-bold text-jetblack">Audit Log Pemanggilan AI</h2>
      <div class="flex items-center gap-2 self-start sm:self-auto">
        ...
        <button
          @click="emit('refresh')"
          class="inline-flex items-center gap-1.5 px-2.5 h-9 rounded-lg bg-jetblack/5 hover:bg-jetblack/10 text-xs font-bold text-jetblack cursor-pointer transition shrink-0"
        >
      ...
      <table class="w-full min-w-full text-left text-xs whitespace-nowrap border-b border-jetblack/15">
        ...
        <tbody class="divide-y divide-jetblack/15">
          ...
          <tr v-for="log in proxyLogs" :key="log.id" class="hover:bg-jetblack/[0.02]">
            ...
            <td class="py-2.5 px-3 text-jetblack">
              <span class="font-bold uppercase text-[10px] mr-1 bg-jetblack/5 px-1.5 py-0.5 rounded">{{ log.provider }}</span>
              <span class="font-mono text-[11px]">{{ log.model }}</span>
            </td>
            <td class="py-2.5 px-3 font-mono font-bold text-jetblack">
              {{ log.totalTokens }} tokens
            </td>
            <td class="py-2.5 px-3 font-mono text-[11px] text-forest font-bold">
              {{ log.latencyMs }} ms
            </td>
            <td class="py-2.5 pl-3 pr-3.5 sm:pr-4 md:pr-6 text-right">
              <span class="inline-flex items-center gap-1 text-[10px] font-bold text-forest bg-forest/10 px-2 py-0.5 rounded-full">
