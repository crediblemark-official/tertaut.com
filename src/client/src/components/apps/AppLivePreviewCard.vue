  ...
  <div class="space-y-4 sticky top-6">
    <div class="border-b border-[#111111]/10 pb-3">
      <div class="flex items-center gap-2">
        ...
        <h2 class="text-xs font-bold uppercase tracking-wider text-[#111111]">Pratinjau Langsung (Checkout)</h2>
      </div>
      <p class="text-[11px] text-[#111111]/60 mt-0.5">Simulasi tampilan checkout pembeli secara real-time.</p>
    </div>

    <!-- Live Preview Card -->
    <div class="p-4 rounded-xl bg-white border border-[#111111]/10 shadow-xs space-y-4">
      <div class="flex items-start gap-3 p-3 rounded-lg bg-[#111111]/[0.02] border border-[#111111]/10">
        ...
        <div class="overflow-hidden">
          <div class="text-xs font-bold text-[#111111] truncate">{{ appName || 'Nama Produk Belum Diset' }}</div>
          <div class="text-[10px] text-[#111111]/60 line-clamp-2 mt-0.5">{{ appDesc || 'Belum ada deskripsi produk.' }}</div>
        </div>
      </div>

      <!-- Benefits in preview -->
      <div class="space-y-1.5 text-xs">
        <div class="text-[10px] uppercase font-bold text-[#111111]/50 tracking-wider">Manfaat Pembeli (Benefits)</div>
        <div v-if="benefits.length > 0" class="space-y-1">
          <div
            v-for="(b, idx) in benefits"
            :key="idx"
            class="flex items-center gap-2 text-[#111111] text-[11px]"
          >
            <Check class="w-3 h-3 text-[#0F4C3A] shrink-0 stroke-[3]" />
            <span class="truncate">{{ b }}</span>
          </div>
        </div>
        <div v-else class="text-[#111111]/40 text-[11px]">Belum ada manfaat ditambahkan</div>
      </div>
      ...
      <div class="space-y-1 text-xs pt-3 border-t border-[#111111]/10">
        ...
        <div class="flex items-baseline justify-between">
          ...
          <span class="text-[11px] font-semibold text-[#111111]/60">
        ...
        <div
          ...
          class="text-[10px] text-[#0F4C3A] font-bold flex items-center gap-1 mt-1 bg-[#0F4C3A]/10 px-2 py-1 rounded-md border border-[#0F4C3A]/20"
        >
        ...
        <div v-if="meteringEnabled" class="mt-2 pt-2 border-t border-dashed border-[#111111]/15">
          ...
          <div class="text-[10px] text-[#111111]/60 font-mono">Rp {{ meteringUnitPrice }} / {{ meteringMetricUnit }}</div>
        </div>
      </div>

      <!-- Status Checkout -->
      <div class="p-3 rounded-lg bg-[#111111]/[0.02] border border-[#111111]/10">
        ...
        <div class="text-[11px] text-[#111111] font-semibold flex items-center justify-between">
          ...
          <span class="text-[#0F4C3A] text-[10px] font-bold">Siap Aktif</span>
        </div>
      </div>
