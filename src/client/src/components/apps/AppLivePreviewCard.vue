...
import { Image as ImageIcon, Check, Sparkles, RefreshCw } from "lucide-vue-next";
...
<template>
  <div class="space-y-4 sticky top-6">
    <div class="border-b border-jetblack/10 pb-3">
      <div class="flex items-center gap-2">
        ...
        <h2 class="text-xs font-bold uppercase tracking-wider text-jetblack">
          Pratinjau Langsung (Checkout)
        </h2>
      </div>
      <p class="text-[11px] text-jetblack/60 mt-0.5">
        Simulasi tampilan checkout pembeli secara real-time.
      </p>
    </div>
    ...
    <div class="p-4 rounded-xl bg-white border border-jetblack/10 shadow-xs space-y-4">
      ...
      >
        <div
          class="w-12 h-12 rounded-md bg-jetblack/5 border border-jetblack/10 flex items-center justify-center overflow-hidden shrink-0"
        >
        ...
        <div class="overflow-hidden">
          <div class="text-xs font-bold text-jetblack truncate">
            {{ appName || "Nama Produk Belum Diset" }}
          </div>
          <div class="text-[10px] text-jetblack/60 line-clamp-2 mt-0.5">
            {{ appDesc || "Belum ada deskripsi produk." }}
          </div>
        </div>
      </div>
      ...
      <div class="pt-2">
        ...
        <p class="text-[10px] text-jetblack/50 text-center mt-2 leading-relaxed">
          Mode {{ dashboardEnv === "sandbox" ? "Sandbox" : "Live" }} — produk ini dan checkout link
          langsung aktif di lingkungan ini.
        </p>
      </div>
    </div>
  </div>
