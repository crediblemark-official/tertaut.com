  ...
  <div class="space-y-4">
    ...
    <div class="grid grid-cols-3 gap-2.5">
      ...
      >
        <div class="flex items-center justify-between">
          ...
          <div
            ...
            :class="pricingType === 'free' ? 'border-gold bg-gold text-jetblack' : 'border-jetblack/30'"
          >
            <Check v-if="pricingType === 'free'" class="w-2.5 h-2.5 stroke-[3]" />
          </div>
        </div>
        <p class="text-[10px]" :class="pricingType === 'free' ? 'text-white/70' : 'text-jetblack/60'">
      ...
      >
        <div class="flex items-center justify-between">
          ...
          <div
            class="w-4 h-4 rounded-full border flex items-center justify-center transition"
            :class="pricingType === 'one_time' ? 'border-gold bg-gold text-jetblack' : 'border-jetblack/30'"
          >
            <Check v-if="pricingType === 'one_time'" class="w-2.5 h-2.5 stroke-[3]" />
          </div>
        </div>
        <p class="text-[10px]" :class="pricingType === 'one_time' ? 'text-white/70' : 'text-jetblack/60'">
      ...
      >
        <div class="flex items-center justify-between">
          ...
          <div
            class="w-4 h-4 rounded-full border flex items-center justify-center transition"
            :class="pricingType === 'subscription' ? 'border-gold bg-gold text-jetblack' : 'border-jetblack/30'"
          >
            <Check v-if="pricingType === 'subscription'" class="w-2.5 h-2.5 stroke-[3]" />
          </div>
        </div>
        <p class="text-[10px]" :class="pricingType === 'subscription' ? 'text-white/70' : 'text-jetblack/60'">
    ...
    <div v-if="pricingType !== 'free'" class="space-y-3">
      ...
      <div v-if="pricingType === 'subscription'" class="space-y-3">
        <div>
          ...
          <div class="relative">
            ...
            >
              ...
              <ChevronDown class="w-4 h-4 text-jetblack/50 transition-transform duration-200" :class="{ 'rotate-180': isBillingPeriodDropdownOpen }" />
            </button>
        ...
        <!-- Custom Days Input -->
        <div v-if="billingPeriod === 'custom'" class="p-3 bg-slate-50 border border-slate-200 rounded-lg space-y-2">
          <label class="block text-[11px] font-bold text-jetblack/70">Interval Penagihan Kustom (Hari)</label>
          <input
    ...
    <div v-if="pricingType === 'subscription'" class="p-3 rounded-xl bg-jetblack/[0.02] border border-jetblack/10 space-y-2.5">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-[10px] text-jetblack/60">Pelanggan tidak ditagih hingga masa trial berakhir.</p>
        </div>
