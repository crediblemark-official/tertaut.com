...
<template>
  <div class="space-y-3">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-2 border-b border-jetblack/10">
    ...
    <div class="-mx-3.5 sm:-mx-4 md:-mx-6 overflow-x-auto top-scrollbar">
      <table class="w-full min-w-full text-left text-xs whitespace-nowrap border-b border-jetblack/15">
        ...
        <tbody class="divide-y divide-jetblack/15">
          <tr v-if="filteredCoupons.length === 0">
            <td colspan="7" class="py-12 px-3.5 sm:px-4 md:px-6 text-center text-jetblack/40">
              <div class="space-y-1.5 max-w-xs mx-auto">
                ...
                <p class="text-[11px] text-jetblack/45">Terbitkan kupon diskon pertama untuk meningkatkan penjualan aplikasi Anda.</p>
                <div class="pt-2">
                  <button
                    type="button"
          ...
          <tr v-for="c in filteredCoupons" :key="c.id" class="hover:bg-jetblack/[0.02] transition-colors">
            ...
            <td class="py-2.5 px-3">
              <span class="inline-flex items-center gap-1 px-1.5 py-0.2 rounded-full bg-gold/15 border border-gold/35 text-jetblack text-[10px] font-bold">
            ...
            <td class="py-2.5 px-3 text-[11px]">
              <span v-if="c.expiresAt" class="inline-flex items-center gap-1" :class="isExpiringSoon(c) ? 'text-crimson font-bold' : 'text-jetblack/60'">
                <Clock class="w-3 h-3" />
