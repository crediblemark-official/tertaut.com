...
<template>
    ...
    <div class="flex items-center justify-between px-3.5 py-2">
      <router-link to="/" class="flex items-center gap-2 group">
        <div class="w-7 h-7 rounded-md bg-[#111111] flex items-center justify-center font-bold text-white text-xs relative shadow-xs">
          T<span class="absolute bottom-0.5 right-0.5 w-1 h-1 rounded-full bg-[#D4AF37]"></span>
        </div>
        <span class="font-extrabold text-xs tracking-tight text-[#111111] font-mono">tertaut<span class="text-[#D4AF37]">.com</span></span>
      </router-link>

      <div class="flex items-center gap-1.5">
        <button type="button" @click="emit('switch-env', env === 'sandbox' ? 'live' : 'sandbox')" class="px-2.5 py-1 min-h-[30px] rounded-lg text-[10.5px] font-bold uppercase tracking-wide border transition flex items-center gap-1 active:scale-95" :class="env === 'sandbox' ? 'bg-[#D4AF37]/15 border-[#D4AF37]/50 text-[#8a6d1f]' : 'bg-[#0F4C3A]/10 border-[#0F4C3A]/40 text-[#0F4C3A]'">
          <span class="w-1.5 h-1.5 rounded-full" :class="env === 'sandbox' ? 'bg-[#D4AF37]' : 'bg-[#0F4C3A]'"></span>
          <span>{{ env === 'sandbox' ? 'Sandbox' : 'Live' }}</span>
        </button>
        <router-link to="/panel" class="p-1.5 min-w-[32px] min-h-[32px] rounded-lg bg-[#111111]/5 hover:bg-[#111111]/10 text-[#111111] text-xs flex items-center justify-center transition" title="Super Admin Panel"><ShieldAlert class="w-4 h-4 text-[#D4AF37]" /></router-link>
        ...
        <router-link v-else to="/login" class="px-2.5 py-1 min-h-[30px] rounded-lg bg-[#111111] text-white text-[11px] font-bold flex items-center gap-1"><LogIn class="w-3.5 h-3.5 text-[#D4AF37]" /><span>Masuk</span></router-link>
      </div>
    </div>
  ...
  <nav class="md:hidden fixed bottom-0 left-0 right-0 h-16 pb-[env(safe-area-inset-bottom,0px)] bg-[#FFFFFF]/95 backdrop-blur-md border-t border-[#111111]/10 flex items-center px-1 z-50 shadow-[0_-4px_20px_rgba(17,17,17,0.08)]">
    <div class="flex items-center w-full justify-around px-0.5">
      <router-link v-for="item in mobileNavItems" :key="item.path" :to="item.path" :class="['flex flex-col items-center justify-center flex-1 h-14 py-1 text-[10px] font-semibold transition-all relative rounded-lg active:scale-95', navKey === item.key ? 'text-[#111111] font-bold' : 'text-[#111111]/50 hover:text-[#111111]']">
        <component :is="item.icon" class="w-4 h-4 mb-1 transition-transform" :class="navKey === item.key ? 'text-[#D4AF37] scale-110' : ''" />
        <span v-if="item.key === '/licensing' && activeLicenseCount > 0" class="absolute top-1 right-[calc(50%-14px)] min-w-[15px] px-1 py-px rounded-full bg-[#0F4C3A] text-white text-[8px] font-black leading-tight text-center shadow-xs">{{ activeLicenseCount }}</span>
        <span class="truncate max-w-[55px] leading-tight text-[9.5px]">{{ item.name }}</span>
        <span v-if="navKey === item.key" class="absolute top-0.5 w-6 h-0.5 bg-[#D4AF37] rounded-full shadow-[0_0_4px_#D4AF37]"></span>
      </router-link>

      <!-- Lainnya -->
      <button type="button" @click="emit('toggle-more')" :class="['flex flex-col items-center justify-center flex-1 h-14 py-1 text-[10px] font-semibold transition-all relative rounded-lg active:scale-95 cursor-pointer', isMoreActive || isMobileMoreOpen ? 'text-[#111111] font-bold' : 'text-[#111111]/50 hover:text-[#111111]']">
        <MoreHorizontal class="w-4 h-4 mb-1 transition-transform" :class="(isMoreActive || isMobileMoreOpen) ? 'text-[#D4AF37] scale-110' : ''" />
        <span v-if="moreBadgeCount > 0 && navKey !== '/coupons'" class="absolute top-1 right-[calc(50%-14px)] min-w-[15px] px-1 py-px rounded-full bg-[#D4AF37] text-[#111111] text-[8px] font-black leading-tight text-center shadow-xs">{{ moreBadgeCount }}</span>
        ...
        <span v-if="isMoreActive || isMobileMoreOpen" class="absolute top-0.5 w-6 h-0.5 bg-[#D4AF37] rounded-full shadow-[0_0_4px_#D4AF37]"></span>
      </button>
    </div>
  </nav>
  ...
  <div v-if="isMobileMoreOpen" class="md:hidden fixed inset-0 z-50 flex flex-col justify-end">
    ...
    <div class="relative bg-white rounded-t-2xl p-4 border-t border-[#111111]/10 shadow-[0_-8px_30px_rgba(0,0,0,0.12)] z-10 space-y-3 pb-[calc(env(safe-area-inset-bottom,0px)+76px)] animate-fadeIn">
      <div class="flex items-center justify-between pb-2 border-b border-[#111111]/10">
        <div class="flex items-center gap-2"><span class="w-1.5 h-4 bg-[#D4AF37] rounded-full"></span><h3 class="text-xs font-bold text-[#111111] uppercase tracking-wider">Menu Tambahan</h3></div>
      ...
      <div class="grid grid-cols-2 gap-2">
        ...
        <router-link :to="envPath(env, '/ai-proxy')" @click="emit('close-more')" class="flex items-center gap-2.5 p-3 rounded-xl border transition-all text-left cursor-pointer active:scale-95" :class="navKey === '/ai-proxy' ? 'bg-[#0F4C3A]/10 border-[#0F4C3A]/40 text-[#111111] font-bold' : 'bg-[#111111]/[0.02] border-[#111111]/10 text-[#111111]/80 hover:bg-[#111111]/5'">
          <div class="w-8 h-8 rounded-lg bg-[#0F4C3A]/15 flex items-center justify-center text-[#0F4C3A] shrink-0"><Bot class="w-4 h-4" /></div>
          <div class="flex-1 min-w-0"><div class="text-xs font-bold leading-tight">AI Proxy Shield</div><div class="text-[10px] text-[#111111]/50 truncate">Audit &amp; Guard</div></div>
        </router-link>
        <router-link :to="envPath(env, '/docs')" @click="emit('close-more')" class="flex items-center gap-2.5 p-3 rounded-xl border transition-all text-left cursor-pointer active:scale-95" :class="navKey === '/docs' ? 'bg-[#111111]/10 border-[#111111]/30 text-[#111111] font-bold' : 'bg-[#111111]/[0.02] border-[#111111]/10 text-[#111111]/80 hover:bg-[#111111]/5'">
          <div class="w-8 h-8 rounded-lg bg-[#111111]/10 flex items-center justify-center text-[#111111] shrink-0"><BookOpen class="w-4 h-4" /></div>
          <div class="flex-1 min-w-0"><div class="text-xs font-bold leading-tight">Dokumentasi</div><div class="text-[10px] text-[#111111]/50 truncate">SDK &amp; API</div></div>
        </router-link>
        <router-link to="/panel" @click="emit('close-more')" class="flex items-center gap-2.5 p-3 rounded-xl border border-[#111111]/10 bg-[#111111]/[0.02] text-[#111111]/80 hover:bg-[#111111]/5 transition-all text-left cursor-pointer active:scale-95">
          <div class="w-8 h-8 rounded-lg bg-[#111111]/10 flex items-center justify-center text-[#111111] shrink-0"><ShieldAlert class="w-4 h-4" /></div>
          <div class="flex-1 min-w-0"><div class="text-xs font-bold leading-tight">Admin Panel</div><div class="text-[10px] text-[#111111]/50 truncate">Super Admin Ledger</div></div>
        </router-link>
      </div>
    </div>
  </div>
