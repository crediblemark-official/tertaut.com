...
function handleKeyDown(e: KeyboardEvent) {
  ...
  if (e.key === "Escape") {
    ...
    const itemIndex =
      props.allOptionLabel && !searchQuery.value
...
<template>
  <div
    :class="[wrapperClass || (buttonClass?.includes('w-full') ? 'w-full block' : 'inline-block')]"
  >
    ...
    >
      <div class="flex items-center gap-2 truncate">
        ...
        <div
          v-if="selectedItem"
          class="w-5 h-5 rounded bg-slate-900 text-white flex items-center justify-center text-[10px] font-bold shrink-0"
        >
          {{ selectedItem.name.charAt(0).toUpperCase() }}
        </div>
        <Search
          v-else
          class="w-3.5 h-3.5 text-slate-400 group-hover:text-amber-500 transition-colors shrink-0"
        />

        <!-- Title & Mode Tag -->
        <div class="flex items-center gap-1.5 truncate">
          <span v-if="selectedItem" class="text-[10px] text-slate-400 font-medium hidden sm:inline"
            >App:</span
          >
      ...
      <div class="flex items-center gap-1 shrink-0 pl-1">
        <kbd
          class="hidden sm:inline-flex items-center gap-0.5 px-1.5 py-0.2 text-[9px] font-mono text-slate-500 bg-slate-100 rounded border border-slate-200"
        >
          <Command class="w-2.5 h-2.5 text-slate-400" />K
        </kbd>
        <ChevronsUpDown
          class="w-3 h-3 text-slate-400 group-hover:text-slate-600 transition-colors"
        />
      </div>
    </button>
        ...
        >
          ...
          >
            ...
            <div class="px-3 py-2 border-b border-slate-100 flex items-center gap-2.5 bg-white">
              ...
              <kbd
                class="px-1.5 py-0.5 text-[9px] font-mono text-slate-500 bg-slate-100 rounded border border-slate-200 shrink-0"
              >
                Esc
              </kbd>
            </div>
            ...
            <div class="overflow-y-auto p-1.5 space-y-0.5 flex-1">
              <div
                class="px-2.5 py-1 text-[9.5px] font-mono font-bold uppercase text-slate-400 tracking-wider"
              >
              ...
              <button
                ...
                :class="[
                  !modelValue
                    ? 'bg-amber-500/10 font-bold text-slate-900'
              ...
              >
                <div class="flex items-center gap-2.5">
                  <div
                    class="w-6 h-6 rounded-md bg-slate-100 flex items-center justify-center text-slate-600 shrink-0"
                  >
                  ...
                  <div>
                    ...
                    <div class="text-[10px] text-slate-500 font-normal leading-tight">
              ...
              <button
                ...
                :class="[
                  modelValue === item.id
                    ? 'bg-amber-500/10 font-semibold text-slate-900'
                    : 'hover:bg-slate-50 text-slate-700',
                  (
                    ...
                    ? 'ring-1.5 ring-amber-500/50 bg-amber-50/40'
                    : '',
                ]"
              >
                <div class="flex items-center gap-2.5 truncate pr-2">
                  <div
                    class="w-6 h-6 rounded-md bg-slate-900 text-white flex items-center justify-center font-bold text-[10px] shrink-0 shadow-2xs"
                  >
                    {{ item.name.charAt(0).toUpperCase() }}
                  </div>
                  <div class="truncate">
                    <div class="font-semibold text-xs text-slate-900 truncate leading-tight">
                      {{ item.name }}
                    </div>
                    <div
                      class="flex items-center gap-1.5 text-[10px] font-mono text-slate-500 leading-tight mt-0.5"
                    >
                      <span v-if="item.slug" class="truncate">{{ item.slug }}</span>
                      <span
                        v-if="item.mode"
                        class="px-1 py-0.2 rounded font-sans uppercase font-bold text-[8px]"
                        :class="
                          item.mode === 'sandbox'
                            ? 'bg-blue-50 text-blue-600 border border-blue-200'
                            : 'bg-emerald-50 text-emerald-700 border border-emerald-200'
                ...
                <div class="flex items-center gap-2 shrink-0">
                  ...
                  <kbd
                    v-else
                    class="text-[9px] font-mono text-slate-400 opacity-0 group-hover:opacity-100"
                    >↵</kbd
                  >
                </div>
              </button>
            ...
            <div
              class="px-3 py-1.5 bg-slate-50/80 border-t border-slate-100 flex items-center justify-between text-[10px] text-slate-500"
            >
              <div class="flex items-center gap-1.5">
                <span
                  ><kbd
                    class="px-1 py-0.2 rounded bg-white border border-slate-200 font-mono text-[9px]"
                    >↑</kbd
                  >
                  <kbd
                    class="px-1 py-0.2 rounded bg-white border border-slate-200 font-mono text-[9px]"
