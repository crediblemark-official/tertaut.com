...
async function handleDeactivate(seat: LicenseSeatItem) {
  ...
  if (seat.leaseActive) {
    if (
      !confirm(
        "Perangkat ini masih memegang lease floating aktif. Lepas seat sekaligus melepas lease?"
      )
    )
      return;
  }
...
<template>
  ...
  >
    <div
      class="bg-white rounded-xl w-full max-w-3xl max-h-[85vh] flex flex-col shadow-xl border border-jetblack/10 animate-fadeIn overflow-hidden"
    >
      <!-- Header -->
      ...
      <div class="flex border-b border-jetblack/10 px-4 gap-4">
        <button
          ...
          :class="[
            'py-2 text-[11px] font-bold transition cursor-pointer relative',
            tab === 'seats' ? 'text-jetblack' : 'text-jetblack/40 hover:text-jetblack/70',
          ]"
        >
        ...
        <button
          ...
          :class="[
            'py-2 text-[11px] font-bold transition cursor-pointer relative',
            tab === 'audit' ? 'text-jetblack' : 'text-jetblack/40 hover:text-jetblack/70',
          ]"
        >
          <span class="inline-flex items-center gap-1">
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        <div class="flex flex-wrap items-center gap-2 text-[10px] font-mono">
          <span class="px-2 py-0.5 rounded-full bg-forest/10 text-forest font-bold">{{
            license.status
          }}</span>
          ...
          >
            <Wifi class="w-2.5 h-2.5 inline mr-0.5 -mt-0.5" /> Floating — TTL lease
            {{ seatsData.leaseTtlSeconds ?? "—" }}s
          </span>
          <span v-else class="px-2 py-0.5 rounded-full bg-jetblack/5 text-jetblack/50">
        ...
        <div
          v-if="errorMsg"
          class="p-2.5 rounded-lg bg-crimson/10 border border-crimson/20 text-crimson text-[11px] font-bold flex items-center gap-1.5"
        >
        ...
        <div class="flex items-center justify-end">
          ...
          >
            <RefreshCw
              class="w-3.5 h-3.5"
              :class="{ 'animate-spin': tab === 'seats' ? loadingSeats : loadingEvents }"
            />
          </button>
        </div>
        ...
        <div v-if="tab === 'seats'">
          <div
            v-if="(seatsData?.seats?.length ?? 0) === 0 && !loadingSeats"
          ...
          >
            ...
            <p class="text-[11px] text-jetblack/35 mt-0.5">
              Seat muncul saat klien melakukan aktivasi (bind hardware).
            </p>
          </div>
          ...
          <div
            ...
            :key="seat.hwidHash"
            class="rounded-lg border border-jetblack/10 p-3 space-y-1.5 mb-2"
          >
            <div class="flex flex-wrap items-center justify-between gap-2">
              <div class="flex items-center gap-2 min-w-0">
                ...
                />
                <span class="text-xs font-bold text-jetblack truncate">{{
                  seat.deviceName || "Perangkat tanpa nama"
                }}</span>
                <code class="text-[10px] text-jetblack/50 font-mono"
                  >{{ seat.hwidHash.substring(0, 10) }}…</code
                >
              </div>
            ...
            <div
              class="grid grid-cols-1 sm:grid-cols-3 gap-1 text-[10px] font-mono text-jetblack/60"
            >
              <span class="inline-flex items-center gap-1"
                ><Wifi class="w-3 h-3" /> Heartbeat: {{ timeAgo(seat.lastHeartbeatAt) }}</span
              >
              <span class="inline-flex items-center gap-1"
                ><Box class="w-3 h-3" /> Terikat: {{ timeAgo(seat.createdAt) }}</span
              >
              <span class="inline-flex items-center gap-1"
                ><Globe class="w-3 h-3" /> IP: {{ seat.ipAddress || "—" }}</span
              >
            </div>
            <div v-if="seat.leaseActive" class="text-[10px] font-mono text-[#7a641a]">
              Lease diperbarui {{ fmt(seat.lastValidatedAt) }} • berakhir
              {{ fmt(seat.leaseExpiresAt) }}
            </div>
          ...
          <ol class="relative border-l border-jetblack/15 ml-1.5 space-y-3">
            <li v-for="ev in events" :key="ev.id" class="pl-4 relative">
              ...
              <div class="flex flex-wrap items-center gap-1.5">
                <span
                  class="px-1.5 py-0.5 rounded bg-jetblack/5 text-jetblack/80 text-[10px] font-mono font-bold"
                >
                  {{ ev.event }}
                </span>
                <span class="text-[10px] text-jetblack/40 font-mono">{{
                  timeAgo(ev.createdAt)
                }}</span>
              </div>
              <p class="text-[11px] text-jetblack/70 mt-0.5">
                {{ EVENT_LABELS[ev.event] || ev.event }}
              </p>
              <p class="text-[10px] font-mono text-jetblack/40">
                <span class="inline-flex items-center gap-1"
                  ><KeyRound class="w-2.5 h-2.5" /> {{ actorLabel(ev) }}</span
                >
