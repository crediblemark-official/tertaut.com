      ...
      <div class="flex items-center gap-2 overflow-x-auto no-scrollbar w-full sm:w-auto">
        ...
        <div class="flex items-center h-7 rounded-md bg-white/10 p-0.5 text-[10px] font-medium shrink-0">
          <button
            ...
            :class="['px-2 h-full rounded transition cursor-pointer flex items-center', platformFilter === 'ALL' ? 'bg-white font-bold text-jetblack' : 'text-white/70 hover:text-white']"
          >
            Semua
          </button>
          <button
            @click="platformFilter = 'desktop'"
            :class="['px-2 h-full rounded transition cursor-pointer flex items-center', platformFilter === 'desktop' ? 'bg-white font-bold text-jetblack' : 'text-white/70 hover:text-white']"
          >
            Desktop
          </button>
          <button
            @click="platformFilter = 'chrome_extension'"
            :class="['px-2 h-full rounded transition cursor-pointer flex items-center', platformFilter === 'chrome_extension' ? 'bg-white font-bold text-jetblack' : 'text-white/70 hover:text-white']"
          >
            Chrome
          </button>
          <button
            @click="platformFilter = 'android'"
            :class="['px-2 h-full rounded transition cursor-pointer flex items-center', platformFilter === 'android' ? 'bg-white font-bold text-jetblack' : 'text-white/70 hover:text-white']"
          >
            Android
          </button>
        </div>
        ...
        <tbody class="divide-y divide-jetblack/15">
          <tr v-if="filteredLicenses.length === 0">
            <td colspan="7" class="py-8 px-3.5 sm:px-4 md:px-6 text-center text-jetblack/40">
              <div class="space-y-1">
                <p class="font-semibold text-xs text-jetblack/60">Tidak ada kunci lisensi ditemukan.</p>
                <p class="text-[11px] text-jetblack/40" v-if="searchQuery || platformFilter !== 'ALL' || statusFilter !== 'ALL'">
          ...
          <tr v-for="lic in filteredLicenses" :key="lic.id" class="hover:bg-jetblack/[0.02]">
            ...
            <td class="py-2.5 px-3 capitalize font-mono text-[11px] text-jetblack/70">{{ lic.platform }}</td>
            <td class="py-2.5 px-3">
              <div
                class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold"
                :class="lic.seatsUsed && lic.seatsUsed >= (lic.maxSeats || 3) ? 'bg-crimson/10 text-crimson' : 'bg-jetblack/5 text-jetblack'"
              >
            ...
            <td class="py-2.5 px-3">
              <span
                ...
                :class="{
                  'bg-forest/10 text-forest': lic.status === 'ACTIVE',
                  'bg-crimson/10 text-crimson': lic.status === 'REVOKED',
                  'bg-jetblack/10 text-jetblack/60': lic.status === 'EXPIRED',
                }"
              >
                {{ lic.status }}
              </span>
