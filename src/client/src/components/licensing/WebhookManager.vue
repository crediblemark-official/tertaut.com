...
<template>
  <div class="animate-fadeIn space-y-3">
    ...
    <div
      class="-mx-3.5 sm:-mx-4 md:-mx-6 px-3.5 sm:px-4 md:px-6 min-h-[44px] py-1.5 sm:py-0 bg-jetblack text-white border-b border-jetblack flex items-center justify-between gap-2 shadow-xs mb-1"
    >
    ...
    <div class="rounded-xl border border-jetblack/10 bg-white shadow-xs p-3.5 sm:p-4 space-y-3">
      <h3
        class="text-[11px] font-bold uppercase tracking-wider text-jetblack/60 flex items-center gap-1.5"
      >
      ...
      <div class="space-y-2.5 text-xs">
        ...
        <div>
          <label class="block font-semibold text-[11px] text-jetblack/70 mb-1">
            ...
            <span class="text-[10px] text-jetblack/40 font-normal"
              >(kosongkan semua = semua event)</span
            >
          </label>
        ...
        <div class="pt-0.5 flex flex-wrap items-center justify-between gap-2">
          ...
          >
            ...
            {{ importAdvancedOpen ? "Sembunyikan" : "Tampilkan" }} Secret Opsional
          </button>
          <button
            @click="createWebhook"
        ...
        <div
          v-if="importAdvancedOpen"
          class="rounded-lg bg-jetblack/[0.03] border border-jetblack/10 p-2.5"
        >
          <label class="block font-semibold text-[11px] text-jetblack/70 mb-1">
          ...
          <p class="mt-1.5 text-[10px] text-jetblack/50 leading-relaxed">
            Setiap delivery ditandatangani header
            <code class="font-mono"
              >x-tertaut-signature: hex hmac-sha256(endpoint.secret, rawBody)</code
            >. Tersedia event berikut:
    ...
    <div
      ...
      class="rounded-xl border border-dashed border-jetblack/20 p-6 text-center"
    >
      <p class="text-xs font-semibold text-jetblack/50">Belum ada webhook endpoint.</p>
      <p class="text-[11px] text-jetblack/35 mt-1">
        Daftarkan endpoint di atas untuk mulai menerima event lifecycle lisensi.
      </p>
    </div>
    ...
    <div
      v-for="w in webhooks"
      :key="w.id"
      class="rounded-xl border border-jetblack/10 bg-white shadow-xs overflow-hidden"
    >
      <div
        class="flex flex-col sm:flex-row sm:items-center gap-2 px-3.5 py-2.5 bg-jetblack/[0.03] border-b border-jetblack/5"
      >
        ...
        <div v-else class="flex flex-wrap items-center justify-between gap-2 pt-1">
          ...
          <div class="flex items-center gap-1.5">
            <template v-if="revealSecretId === w.id">
              <code
                class="text-[10px] font-mono text-[#7a641a] bg-gold/10 px-1.5 py-0.5 rounded max-w-40 truncate"
              >
                {{ w.secret }}
              </code>
              <button
