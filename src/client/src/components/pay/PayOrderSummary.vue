...
<template>
  ...
  <div
    class="hidden lg:flex flex-col justify-between p-6 xl:p-7 border-r border-slate-200 bg-slate-50/70 h-full overflow-y-auto custom-scrollbar shrink-0"
  >
    <div class="space-y-3.5 my-auto py-1">
      ...
      <div v-if="product.valueProps && product.valueProps.length > 0" class="space-y-2 py-0.5">
        <div
          v-for="(vp, idx) in product.valueProps.slice(0, 3)"
          :key="idx"
      ...
      <div class="space-y-1.5 pt-3 border-t border-slate-200">
        ...
        <p class="text-[11px] text-slate-600 leading-relaxed font-normal">
          Kunci lisensi resmi dan petunjuk aktivasi otomatis dikirimkan ke alamat email ini setelah
      ...
      <div class="space-y-2.5 text-xs sm:text-sm pt-3 border-t border-slate-200">
        ...
        <div class="pt-1">
          ...
          <p v-if="couponError" class="text-xs text-red-600 font-semibold mt-1">
            {{ couponError }}
          </p>
          <div
  ...
  <div class="lg:hidden rounded-xl border border-slate-200 bg-slate-50/80 p-3.5 space-y-2.5">
    <div class="flex items-center justify-between">
      <div>
        <div class="text-[10px] font-bold uppercase tracking-wider text-amber-600 font-mono">
          Pesanan Software
        </div>
        <h1 class="text-sm sm:text-base font-extrabold text-slate-950 leading-tight">
          {{ product.name }}
        </h1>
      </div>
    ...
    <div
      v-if="isMobileOrderExpanded"
      class="pt-2 border-t border-slate-200/80 space-y-2.5 animate-in fade-in duration-150"
    >
