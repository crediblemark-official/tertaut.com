  ...
  <div class="space-y-3">
    ...
    >
      <div class="flex items-center gap-2">
        <div class="flex items-center gap-1.5">
          ...
          <h1 class="text-xs font-bold uppercase tracking-wider text-white">Riwayat Transaksi & Pembayaran</h1>
        </div>
    ...
    <div
      ...
      :class="[
        ...
        disburseAlert.startsWith('Error') || disburseAlert.startsWith('Peringatan') || disburseAlert.startsWith('Gagal')
    ...
    >
      <div class="flex items-center gap-2">
        <component
          :is="disburseAlert.startsWith('Error') || disburseAlert.startsWith('Peringatan') || disburseAlert.startsWith('Gagal') ? AlertTriangle : CheckCircle2"
    ...
    <div class="-mx-3.5 sm:-mx-4 md:-mx-6 px-3.5 sm:px-4 md:px-6 grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 py-2.5 sm:py-3 border-b border-jetblack/15 bg-white">
          ...
          <tr v-if="filteredTransactions.length === 0">
            <td colspan="10" class="py-10 px-3.5 sm:px-4 md:px-6 text-center text-jetblack/40">
              <div class="space-y-1">
                ...
                <p class="font-semibold text-xs text-jetblack/70">Tidak ada riwayat transaksi ditemukan.</p>
    ...
    <!-- Pagination Bar -->
    <div class="-mx-3.5 sm:-mx-4 md:-mx-6 px-3.5 sm:px-4 md:px-6 py-2.5 border-b border-jetblack/15 bg-white flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
      ...
      <div class="bg-white border border-jetblack/15 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-scaleIn flex flex-col max-h-[90vh]">
        ...
        <div class="px-5 py-4 overflow-y-auto space-y-4 text-xs">
          ...
          <div class="space-y-2.5">
            ...
            <div v-if="selectedTx.couponCode" class="flex justify-between py-1 border-b border-slate-100">
              <span class="text-jetblack/60">Kupon Diskon</span>
              <span class="font-mono font-bold text-jetblack bg-gold/15 px-1.5 py-0.2 rounded border border-gold/30">
            ...
            <div class="flex justify-between py-1 border-b border-slate-100">
              ...
              <span
                class="font-semibold"
                :class="selectedTx.disbursementStatus === 'COMPLETED' ? 'text-forest' : 'text-slate-600'"
              >
                {{ selectedTx.disbursementStatus === 'COMPLETED' ? 'Sudah Ditransfer' : 'Menunggu Pencairan' }}
              </span>
            </div>
            <div class="flex justify-between py-1 border-b border-slate-100">
          ...
          <div
            v-if="selectedTx.paymentStatus === 'PAID' && selectedTx.disbursementStatus !== 'COMPLETED'"
          ...
          >
            ...
            <div class="space-y-1">
              ...
              <p class="text-[11px] text-jetblack/60">
                Net builder <span class="font-mono font-bold text-forest">{{ formatRupiah(selectedTx.netAmount) }}</span> dari transaksi ini masuk ke akumulasi saldo Anda.
                Penarikan dana dilakukan secara agregat dari seluruh software di halaman Balances.
              </p>
              <router-link
                :to="envPath(env, '/balances')"
          ...
          <div v-if="env === 'sandbox' && selectedTx.paymentStatus === 'PENDING'" class="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 space-y-2">
            ...
            <p class="text-[11px] text-amber-800/80">
              Uji coba konfirmasi pembayaran klien Anda tanpa dana nyata untuk otomatis menerbitkan lisensi dan memverifikasi webhook.
            </p>
            <button
              type="button"
