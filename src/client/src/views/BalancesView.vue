...
async function loadData() {
  loading.value = true;
  try {
    const [txRes] = await Promise.all([api.getTransactions()]);
...
const balanceKPIs = computed(() => {
  ...
  const availableTxs = paidTxs.filter(
    (t) => t.disbursementStatus === "PENDING" || !t.disbursementStatus
  );
...
const disbursementGroups = computed((): DisbursementGroup[] => {
  const paidTxs = transactions.value.filter((t) => t.paymentStatus === "PAID");
  const disbursed = paidTxs.filter(
    (t) =>
...
async function handleRequestPayout() {
  ...
  if (env.value === "sandbox") {
    payoutError.value =
      "Pencairan dana nyata hanya tersedia di mode LIVE. Di mode Sandbox, saldo bersifat simulasi.";
    return;
  }
...
<template>
  <div class="animate-fadeIn pb-12">
    ...
    >
      ...
      <div class="flex items-center gap-2 overflow-x-auto no-scrollbar w-full sm:w-auto">
        ...
        <button
          ...
          :title="
            env === 'sandbox'
              ? 'Pencairan dana hanya tersedia di mode LIVE'
    ...
    >
      <div class="space-y-0.5">
        <div class="text-xs font-semibold text-jetblack/60 flex items-center gap-1.5">
          ...
          <span
            v-if="env === 'sandbox'"
            class="text-[9px] px-1.5 rounded bg-amber-500/15 text-amber-800 font-bold uppercase"
            >Simulasi</span
          >
        </div>
    ...
    >
      ...
      >
        <div class="flex items-center gap-3">
          ...
          >
            <ArrowDownLeft
              :class="env === 'sandbox' ? 'w-4 h-4 text-amber-600' : 'w-4 h-4 text-forest'"
            />
          </div>
          <div>
    ...
    <div
      class="-mx-3.5 sm:-mx-4 md:-mx-6 px-3.5 sm:px-4 md:px-6 flex items-center justify-between py-1.5 border-b border-jetblack/10"
    >
      <div class="flex items-center gap-2">
        <Landmark class="w-3.5 h-3.5 text-jetblack/50" />
        <span class="text-[11px] font-bold uppercase tracking-wider text-jetblack/60"
          >Riwayat Pencairan</span
        >
        <span
          class="text-[10px] px-1.5 py-0.5 rounded-full bg-jetblack/8 text-jetblack/60 font-mono font-bold"
        >
      ...
      <div class="relative w-64">
        <Search
          class="w-3 h-3 absolute left-2.5 top-1/2 -translate-y-1/2 text-jetblack/30 pointer-events-none"
        />
        <input
    ...
    <div class="-mx-3.5 sm:-mx-4 md:-mx-6 overflow-x-auto top-scrollbar">
      ...
      >
        ...
        <tbody class="divide-y divide-jetblack/15">
          ...
          >
            <td
              class="py-3 pr-3 pl-3.5 sm:pl-4 md:pl-6 font-mono font-bold text-jetblack text-[11px]"
            >
            ...
            <td
              class="py-3 pl-3 pr-3.5 sm:pr-4 md:pr-6 text-right text-[11px] text-jetblack/60 font-mono"
            >
    ...
    >
      <div
        class="bg-white border border-jetblack/15 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-scaleIn flex flex-col"
      >
        <div
        ...
        <form @submit.prevent="handleRequestPayout" class="p-5 space-y-4 text-xs">
          ...
          <div class="p-3.5 rounded-xl bg-slate-50 border border-slate-200/80 space-y-1">
            <span class="text-[11px] text-jetblack/60 font-semibold"
              >Saldo Bersih Siap Ditarik</span
            >
          ...
          >
            ...
            <div>
              ...
              <div class="text-[11px] text-jetblack/60 mt-0.5">
                Dana akan langsung ditransfer ke rekening bank / DANA bisnis yang terdaftar di
                profil builder Anda.
              </div>
            </div>
          </div>
          ...
          <div
            v-if="payoutError"
            class="p-3 rounded-lg bg-red-50 border border-red-200 text-red-700 text-xs flex items-center gap-2"
          >
          ...
          <div
            v-if="payoutSuccess"
            class="p-3 rounded-lg bg-forest/10 border border-forest/20 text-forest text-xs flex items-center gap-2"
          >
          ...
          <div class="pt-2 flex items-center justify-end gap-2 border-t border-jetblack/10">
            ...
            <button
              type="submit"
              :disabled="
                isSubmittingPayout || balanceKPIs.availableBalance < 50000 || payoutAmount < 50000
              "
    ...
    >
      <div
        class="w-full max-w-md bg-white border border-jetblack/15 rounded-2xl shadow-2xl p-5 space-y-4"
      >
        <div class="flex items-center justify-between border-b border-jetblack/10 pb-3">
          <div class="flex items-center gap-2">
            <Building2 class="w-4 h-4 text-gold" />
            <h3 class="text-xs font-bold uppercase tracking-wider text-jetblack">
              Rekening Pencairan Builder
            </h3>
          </div>
          <button
            @click="isAccountModalOpen = false"
            class="p-1 text-jetblack/40 hover:text-jetblack transition cursor-pointer"
          >
            <X class="w-4 h-4" />
          </button>
        </div>
        ...
        <form @submit.prevent="handleSaveAccount" class="space-y-3 text-xs">
          ...
          <div>
            <label class="block text-[11px] font-bold text-jetblack/70 mb-1"
              >Nama Pemilik Rekening Sesuai Buku Tabungan</label
            >
            <input
          ...
          <div
            v-if="accountFeedback"
            class="p-2.5 rounded-lg bg-forest/10 border border-forest/20 text-forest text-xs font-bold flex items-center gap-2"
          >
            <CheckCircle2 class="w-4 h-4 shrink-0" />
