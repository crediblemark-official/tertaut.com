...
async function createCheckout() {
  if (!selectedAppId.value) {
    disburseAlert.value =
      "Peringatan: Silakan buat atau pilih software terlebih dahulu sebelum membuat link checkout.";
    return;
  }
  if (!customerEmail.value || !customerEmail.value.includes("@")) {
    disburseAlert.value =
      "Peringatan: Silakan masukkan email pembeli yang valid (contoh: buyer@example.com).";
    return;
  }
  loading.value = true;
  disburseAlert.value = null;
  ...
  <div class="animate-fadeIn pb-8">
    ...
    <div
      ...
      :class="[
        ...
        disburseAlert.startsWith('Silakan')
    ...
    <div v-if="activeTab === 'generator'" class="space-y-3.5 animate-fadeIn">
      ...
      >
        <div class="flex items-center gap-2 truncate">
          ...
          <div class="truncate">
            ...
            <span class="font-mono text-gold ml-1.5 font-bold"
              >tertaut.com/pay/{{ currentApp.slug }}</span
            >
          </div>
        </div>
      ...
      <div
        class="grid grid-cols-1 lg:grid-cols-12 divide-y lg:divide-y-0 lg:divide-x divide-jetblack/10 border-t border-jetblack/10 pt-3.5"
      >
