  ...
  <div class="animate-fadeIn pb-12">
      ...
      <div class="flex items-center gap-2 overflow-x-auto no-scrollbar w-full sm:w-auto">
        ...
        <div class="flex items-center h-7 rounded-md bg-white/10 p-0.5 text-[10px] font-medium shrink-0">
          ...
          <button
            ...
            :class="['px-2 h-full rounded transition cursor-pointer flex items-center', selectedStatus === 'PAID' ? 'bg-white font-bold text-[#111111]' : 'text-white/70 hover:text-white']"
          >
            Paid
          </button>
          <button
            @click="selectedStatus = 'PENDING'"
            :class="['px-2 h-full rounded transition cursor-pointer flex items-center', selectedStatus === 'PENDING' ? 'bg-white font-bold text-[#111111]' : 'text-white/70 hover:text-white']"
          >
            Pending
          </button>
          <button
            @click="selectedStatus = 'FAILED'"
            :class="['px-2 h-full rounded transition cursor-pointer flex items-center', selectedStatus === 'FAILED' ? 'bg-white font-bold text-[#111111]' : 'text-white/70 hover:text-white']"
          >
            Failed
          </button>
          <button
    ...
    <div class="-mx-3.5 sm:-mx-4 md:-mx-6 px-3.5 sm:px-4 md:px-6 grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 py-2.5 sm:py-3 border-b border-[#111111]/15 mb-0">
      <div class="space-y-0.5">
        <div class="text-xs font-semibold text-[#111111]/60">Gross Volume</div>
        <div class="text-2xl font-bold text-[#111111] font-mono tracking-tight">{{ formatRupiah(kpiStats.totalGross) }}</div>
        <div class="text-[11px] text-[#111111]/50 font-medium">{{ kpiStats.paidCount }} transaksi sukses</div>
      </div>

      <div class="space-y-0.5 sm:border-l sm:border-[#111111]/10 sm:pl-5">
        <div class="text-xs font-semibold text-[#111111]/60">Net Creator (95%)</div>
        <div class="text-2xl font-bold text-[#0F4C3A] font-mono tracking-tight">{{ formatRupiah(kpiStats.totalNet) }}</div>
        <div class="text-[11px] text-[#111111]/50 font-medium">Fee MoR Tertaut 5% ({{ formatRupiah(kpiStats.totalFee) }})</div>
      </div>

      <div class="space-y-0.5 sm:border-l sm:border-[#111111]/10 sm:pl-5">
        <div class="text-xs font-semibold text-[#111111]/60">Transaksi Berhasil</div>
        <div class="text-2xl font-bold text-[#111111] font-mono tracking-tight">{{ kpiStats.paidCount }}</div>
        <div class="text-[11px] text-[#111111]/50 font-medium">dari total {{ kpiStats.totalCount }} pesanan</div>
      </div>

      <div class="space-y-0.5 sm:border-l sm:border-[#111111]/10 sm:pl-5">
        <div class="text-xs font-semibold text-[#111111]/60">Menunggu Pembayaran</div>
        <div class="text-2xl font-bold text-[#D4AF37] font-mono tracking-tight">{{ kpiStats.pendingCount }}</div>
        <div class="text-[11px] text-[#111111]/50 font-medium">menunggu konfirmasi webhook</div>
      </div>
    </div>
          ...
          >
            <td class="py-3 pr-3 pl-3.5 sm:pl-4 md:pl-6 font-mono font-bold text-[#111111]">
              <div class="flex items-center gap-1.5">
                ...
                <span
                  v-if="env === 'sandbox'"
                  class="px-1.5 py-0.2 rounded text-[9px] font-mono font-bold bg-[#D4AF37]/15 text-[#8a6d1f] border border-[#D4AF37]/30"
                >
                  SIM
                </span>
              </div>
            ...
            <td class="py-3 px-3">
              <div class="font-medium text-[#111111]">{{ tx.customerEmail || '-' }}</div>
            </td>
            <td class="py-3 px-3">
              <span class="font-medium text-[#111111]/80">{{ getAppName(tx.appId) }}</span>
            </td>
            ...
            <td class="py-3 px-3">
              <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10.5px] font-medium bg-slate-100 text-slate-700">
                <CreditCard class="w-3 h-3 text-[#D4AF37]" />
            ...
            <td class="py-3 px-3">
              ...
              >
                <span
                  ...
                  :class="tx.paymentStatus === 'PAID' ? 'bg-[#0F4C3A]' : tx.paymentStatus === 'PENDING' ? 'bg-[#D4AF37]' : 'bg-red-500'"
                ></span>
                {{ tx.paymentStatus }}
              </span>
            </td>
    ...
    >
      <div class="bg-white border border-[#111111]/15 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-scaleIn flex flex-col max-h-[90vh]">
        <!-- Header -->
        ...
        <div class="px-5 py-4 overflow-y-auto space-y-4 text-xs">
          ...
          <div class="p-3.5 rounded-xl bg-slate-50 border border-slate-200/80 space-y-2">
            <div class="flex items-center justify-between">
              <span class="text-[11px] text-[#111111]/60 font-semibold">Total Nilai Pembayaran</span>
              <span
                :class="selectedTx.paymentStatus === 'PAID' ? 'bg-[#0F4C3A]/10 text-[#0F4C3A] border-[#0F4C3A]/20' : 'bg-amber-500/10 text-amber-700 border-amber-500/20'"
            ...
            <div class="pt-2 border-t border-slate-200 grid grid-cols-2 gap-2 text-[11px]">
              <div>
                <span class="text-[#111111]/50 block">Platform Fee (5% MoR):</span>
                <span class="font-mono font-bold text-red-600">- {{ formatRupiah(selectedTx.platformFee) }}</span>
              </div>
              <div>
                <span class="text-[#111111]/50 block">Net Payout (95%):</span>
                <span class="font-mono font-bold text-[#0F4C3A]">{{ formatRupiah(selectedTx.netAmount) }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="space-y-2.5">
            <div class="flex justify-between py-1 border-b border-slate-100">
              <span class="text-[#111111]/60">ID Transaksi</span>
              <span class="font-mono font-semibold text-[#111111]">{{ selectedTx.id }}</span>
            </div>
            <div class="flex justify-between py-1 border-b border-slate-100">
              <span class="text-[#111111]/60">Pelanggan</span>
              <span class="font-semibold text-[#111111]">{{ selectedTx.customerEmail }}</span>
            </div>
            <div class="flex justify-between py-1 border-b border-slate-100">
              <span class="text-[#111111]/60">Produk</span>
              <span class="font-semibold text-[#111111]">{{ getAppName(selectedTx.appId) }}</span>
            </div>
            <div class="flex justify-between py-1 border-b border-slate-100">
              <span class="text-[#111111]/60">Kanal Pembayaran</span>
              <span class="font-mono font-semibold text-[#111111] uppercase">{{ selectedTx.paymentChannel || 'QRIS' }}</span>
            </div>
            <div class="flex justify-between py-1 border-b border-slate-100">
              <span class="text-[#111111]/60">Gateway / MoR Provider</span>
              <span class="font-semibold text-[#111111] uppercase">{{ selectedTx.paymentProvider || 'Xendit' }}</span>
            </div>
            <div class="flex justify-between py-1 border-b border-slate-100">
              <span class="text-[#111111]/60">Status Pencairan (Disbursement)</span>
              <span class="font-semibold text-[#111111] uppercase">{{ selectedTx.disbursementStatus || 'PENDING' }}</span>
            </div>
            <div class="flex justify-between py-1 border-b border-slate-100">
              <span class="text-[#111111]/60">Waktu Pembayaran</span>
              <span class="text-[#111111]">{{ formatDate(selectedTx.paidAt || selectedTx.createdAt) }}</span>
            </div>
          </div>
          ...
          <div v-if="env === 'sandbox' && selectedTx.paymentStatus === 'PENDING'" class="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 space-y-2">
            ...
            <div v-if="simulateMsg" class="text-xs font-bold text-[#0F4C3A] text-center pt-1">
              {{ simulateMsg }}
            </div>
          </div>
        </div>
        ...
        <!-- Footer -->
        <div class="px-5 py-3 border-t border-[#111111]/10 bg-slate-50/70 shrink-0 flex items-center justify-end">
          <button
            type="button"
            @click="closeDetail"
