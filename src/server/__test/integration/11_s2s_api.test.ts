...
describe("S2S Server-to-Server API (secret API key)", () => {
  ...
  it("end-to-end: info, apps, issue, saldo, konsumsi idempoten, list, revoke", async () => {
    ...
    const [b] = await db
      .insert(builders)
      .values({ email, name: "S2S Tester", apiKey: generateAppApiKey("live"), secretApiKey: secret })
      .returning();
    builderEmail = email;
    ...
    const issueRes = await (await postJson("/licenses/issue", {
      appId,
    ...
    // Saldo kredit
    const bal = await (await fetch(`${BASE}/credits/balance?licenseKey=${licKey}`, { headers: authHeaders() })).json();
    expect(bal.balance).toBe(100);

    // Konsumsi 5 kredit
    const consume = await (await postJson("/credits/consume", {
      licenseKey: licKey,
      amount: 5,
      reference: "call-123",
    })).json();
    ...
    const consumeAgain = await (await postJson("/credits/consume", {
      licenseKey: licKey,
      amount: 5,
      reference: "call-123",
    })).json();
    ...
    const listLic = await (await fetch(`${BASE}/licenses?appId=${appId}`, { headers: authHeaders() })).json();
    ...
    const evil = await (await fetch(`${BASE}/credits/balance?licenseKey=${licKey}`, {
    ...
    const otherAppsRes = await (await fetch(`${BASE}/apps?mode=live`, {
      headers: { Authorization: `Bearer ${other.secretApiKey}` },
    })).json();
