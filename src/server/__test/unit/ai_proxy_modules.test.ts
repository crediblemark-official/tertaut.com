...
describe("AI Proxy Upstream Caller & Utilities", () => {
  ...
  it("should execute callUpstreamNonStreaming across all supported providers", async () => {
    ...
    const geminiRes = await callUpstreamNonStreaming(
      "gemini",
      "gemini-1.5-flash",
      "test-key",
      "Halo"
    ...
    globalThis.fetch = (async (url: string) => {
      if (url.includes("api.anthropic.com")) {
        return new Response(JSON.stringify({ content: [{ text: "Claude response" }] }), {
          status: 200,
        });
      }
      return new Response("Not found", { status: 404 });
    }) as any;
    const anthropicRes = await callUpstreamNonStreaming(
      "anthropic",
    ...
    globalThis.fetch = (async (url: string) => {
      if (url.includes("api.deepseek.com")) {
        return new Response(
          JSON.stringify({ choices: [{ message: { content: "DeepSeek response" } }] }),
          { status: 200 }
        );
      }
      return new Response("Not found", { status: 404 });
    }) as any;
    const deepseekRes = await callUpstreamNonStreaming(
      "deepseek",
      "deepseek-chat",
      "test-key",
      "Halo"
  ...
  it("should throw formatted errors when upstream provider fails in non-streaming", async () => {
    ...
    expect(callUpstreamNonStreaming("gemini", "gemini-1.5-flash", "key", "hi")).rejects.toThrow(
      "Gemini upstream error 429"
    );
    expect(callUpstreamNonStreaming("openai", "gpt-4o", "key", "hi")).rejects.toThrow(
      "OpenAI upstream error 429"
    );
    expect(callUpstreamNonStreaming("anthropic", "claude-3", "key", "hi")).rejects.toThrow(
      "Anthropic upstream error 429"
    );
    expect(callUpstreamNonStreaming("deepseek", "deepseek-chat", "key", "hi")).rejects.toThrow(
      "DeepSeek upstream error 429"
    );
    expect(callUpstreamNonStreaming("unsupported" as any, "m", "k", "h")).rejects.toThrow(
      "tidak didukung"
    );
  });
  ...
  it("should stream and parse chunks across all providers in callUpstreamStreamingChunks", async () => {
    ...
    const geminiStream =
      "data: " +
      JSON.stringify({ candidates: [{ content: { parts: [{ text: "Gemini chunk" }] } }] }) +
      "\n\n";
    globalThis.fetch = (async () => new Response(geminiStream, { status: 200 })) as any;
    const geminiChunks = await callUpstreamStreamingChunks(
      "gemini",
      "gemini-1.5-flash",
      "key",
      "test"
    ...
    const openaiStream =
      "data: " +
      JSON.stringify({ choices: [{ delta: { content: "OpenAI chunk" } }] }) +
      "\n\ndata: [DONE]\n\n";
    ...
    const anthropicStream =
      "data: " +
      JSON.stringify({ type: "content_block_delta", delta: { text: "Anthropic chunk" } }) +
      "\n\n";
    globalThis.fetch = (async () => new Response(anthropicStream, { status: 200 })) as any;
    const anthropicChunks = await callUpstreamStreamingChunks(
      "anthropic",
      "claude-3",
      "key",
      "test"
    ...
    const deepseekStream =
      "data: " + JSON.stringify({ choices: [{ delta: { content: "DeepSeek chunk" } }] }) + "\n\n";
    globalThis.fetch = (async () => new Response(deepseekStream, { status: 200 })) as any;
    const deepseekChunks = await callUpstreamStreamingChunks(
      "deepseek",
      "deepseek-chat",
      "key",
      "test"
  ...
  it("should throw formatted errors when upstream provider fails in streaming", async () => {
    ...
    expect(
      callUpstreamStreamingChunks("gemini", "gemini-1.5-flash", "key", "test")
    ).rejects.toThrow("Gemini upstream error 400");
    expect(callUpstreamStreamingChunks("openai", "gpt-4o", "key", "test")).rejects.toThrow(
      "OpenAI upstream error 400"
    );
    expect(callUpstreamStreamingChunks("anthropic", "claude-3", "key", "test")).rejects.toThrow(
      "Anthropic upstream error 400"
    );
    expect(callUpstreamStreamingChunks("deepseek", "deepseek-chat", "key", "test")).rejects.toThrow(
      "DeepSeek upstream error 400"
    );
    expect(callUpstreamStreamingChunks("unknown" as any, "m", "k", "t")).rejects.toThrow(
      "tidak didukung"
    );
  });
...
describe("AI App Config, Vault, and Logs Handlers", () => {
  ...
  it("should handle save and get AI app configs (create & update)", async () => {
    ...
    const notFoundRes = await handleSaveAppConfig({
      body: { appId: "non_existent_app_999", modelAlias: "default" },
      set,
    });
  ...
  it("should handle AI Vault credentials and Kill Switch toggling", async () => {
    ...
    const errRes = await saveVaultCredential(
      { appId: "invalid_app_999", provider: "openai", rawApiKey: "sk-123" },
      set
    );
    expect(set.status).toBe(404);
    ...
    const saveRes = await saveVaultCredential(
      {
        appId: testAppId,
        provider: "anthropic",
        rawApiKey: "sk-ant-test-key-9999",
    ...
    const updateCredRes = await saveVaultCredential(
      {
        appId: testAppId,
        provider: "anthropic",
        rawApiKey: "sk-ant-test-key-updated",
        monthlyBudgetLimit: 1200000,
      },
      {}
    );
    ...
    const toggleNotFound = await handleToggleKillSwitch({
      body: { appId: testAppId, provider: "non_existent" },
      set,
    });
    expect(set.status).toBe(404);

    const toggleOn = await handleToggleKillSwitch({
      body: { appId: testAppId, provider: "anthropic" },
      set: {},
    });
    expect(toggleOn.success).toBe(true);
    expect(toggleOn.isKillSwitchActive).toBe(true);

    const toggleOff = await handleToggleKillSwitch({
      body: { appId: testAppId, provider: "anthropic" },
      set: {},
    });
